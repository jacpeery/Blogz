...
def login():
    ...
    if request.method == 'POST':
        ...
        if not user:
            username_error = "Pawsitive this user doesn't exist, yet."
            if username == "":
                username_error = "Paws! Enter your username."
        
        if password == "":
            password_error = "Paws! Enter your password."
        
        if user and user.password != password:
            password_error = "Paws! Incorrect password."
...
def signup():
    ...
    if request.method == 'POST':
        ...
        if len(username) < 3:
            username_error = "Paws! Username must be greater than 3 characters."
            if username == "":
                username_error = "Paws! Enter a username."
       
        if password != verify:
            password_error = "Paws! Creative passwords must match!"
            verify_error = "Paws! Creative passwords must match!"

        if len(password) < 3:
            password_error = "Paws! Password must be greater than 3 characters."
            if password == "":
                password_error = "Paws! Enter a creative password "
        ...
        if not username_error and not password_error and not verify_error:
            if not exisiting_user:
                ...
                username_error = "Paws! Username territory marked."
...
def create_new_post():
    ...
    if request.method == 'POST':
        ...
        if blog_title == "":
            title_error = "Paws! Enter a doggy bloggy title."

        if blog_content == "":
            content_error = "Paws! Write a puppy post."
    ...
    return render_template('newpost.html', title = "Add a new puppy post", blog_title = blog_title, blog_content = blog_content, title_error = title_error, content_error = content_error)
